...
function M.new_chat()
	--Save chat and rest window
	if last_saved_chat_name then
		require("CopilotChat").save(last_saved_chat_name)
	elseif M.is_chat_buffer_empty() then
		M.save_window()
	end
	require("CopilotChat").reset()
	M.reset_saved_chat_name()
end
function M.is_chat_buffer_empty()
	local bufnr = vim.fn.bufnr("CopilotChat")
	if bufnr == -1 then
		return true
	end
